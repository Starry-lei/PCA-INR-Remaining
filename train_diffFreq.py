...
from dataset.data_loader_res import PCDataset,RandomPairSampler,paired_collate_fn
from dataset.data_loader_res import PCDValataset
...
def train(args):
    ...
    # 回头封装一下上面的东西
    num_locals=4
    flow_lat_global_params = torch.nn.Parameter(torch.randn(dataset_train.data_length, args.lat_dims, device=device) * 1e-1, requires_grad=True, )
    flow_lat_params = torch.nn.Parameter(torch.randn(dataset_train.data_length, num_locals, args.lat_dims, device=device) * 1e-1, requires_grad=True, )
    # flow_lat_high_freq_params = torch.nn.Parameter(torch.randn(dataset_train.data_length, args.lat_high_freq_dims, device=device) * 1e-1, requires_grad=True, )

    
    model.add_lat_params(flow_lat_params, 'flow_lat_params')
    model.add_lat_params(flow_lat_global_params, 'flow_lat_global_params')
    # model.add_lat_params(flow_lat_high_freq_params, 'flow_lat_high_freq_params')
    ...
    for epoch in range(num_epochs):
        ...
        for data_tensors  in tqdm_train_loader:
            ...
            flow_global_source=flow_lat_global_params[ii]
            flow_global_target=flow_lat_global_params[jj]
            mean_latents_global= torch.zeros_like(flow_global_source)
            source_target_global_latents = torch.cat([mean_latents_global, flow_global_source], dim=0)
            target_source_global_latents=torch.cat([flow_global_target, mean_latents_global], dim=0)

            global_latent_seq=torch.stack([source_target_global_latents, target_source_global_latents], dim=1)
            
            # flow_lat_highfreq_source= flow_lat_high_freq_params[ii]
            # flow_lat_highfreq_target= flow_lat_high_freq_params[jj]

            # print("shape of flow_lat_source:", flow_lat_source.shape) # torch.Size([4, 12])
            # print("shape of flow_lat_target:", flow_lat_target.shape) # torch.Size([4, 12])
            # print("shape of low_lat_highfreq_source:", low_lat_highfreq_source.shape) # torch.Size([4, 8])
            # print("shape of flow_lat_highfreq_target:", flow_lat_highfreq_target.shape) # torch.Size([4, 8])
            ...
            # source_residual_latents= torch.cat([flow_lat_highfreq_source, flow_lat_highfreq_target], dim=0)

            # print("show shape of source_residual_latents:",source_target_latents.shape)# torch.Size([8, 4, 12])
            # print("show shape of source_target_latents:",source_target_latents.shape)
            # # 8,8
            # exit()
            mean_latents_theta=torch.zeros_like(source_thetas)
            target_source_latents = torch.cat([flow_lat_target, mean_latents], dim=0)
            g_source_target_latents = torch.cat([mean_latents_theta, source_thetas], dim=0)
            g_target_source_latents = torch.cat([target_theta, mean_latents_theta], dim=0)
            ...
            # print("show latent_seq:",latent_seq.shape)  # 8, 8, 12 for 4 locals, 4 vs 4, torch.Size([8, 2, 12]) for global latent vector, 1 vs 1
            # exit()       
            deformed_pts, interpo_points_basis_transformed = model(source_target_points[..., :3], latent_seq, pca_guidance_latents=g_latent_seq, shape_name=source_names,
                                                                   
                                                                   source_residual_latents= None,
                                                                   global_latent_seq=global_latent_seq
                                                                                                                                
                                                                   )  
            # print("see epoch:",epoch_idx)
            # print("see epoch % 100:",epoch_idx % 100)
            deformed_pts_global= deformed_pts[0]
            deformed_pts_local=  deformed_pts[1]

            
            main_deformed_pts_global= deformed_pts_global[-2*source_pts.shape[0]:,:]
            reg_deformed_pts_global= deformed_pts_global[:-2*source_pts.shape[0],:]

            main_deformed_pts_local= deformed_pts_local[-2*source_pts.shape[0]:,:]
            reg_deformed_pts_local= deformed_pts_local[-2*source_pts.shape[0]:,:]
            ...
            if epoch_idx % 50 == 0:
                ...
                deformed_pts_vis= main_deformed_pts_local[0]
            ...
            cd_l1_global, _= calc_cd(main_deformed_pts_global, target_source_points)
            loss_mse_global = mean_flat(criterion_mse(main_deformed_pts_global, target_source_points)) 

            cd_l1_local, _= calc_cd(main_deformed_pts_local, target_source_points)
            loss_mse_local = mean_flat(criterion_mse(main_deformed_pts_local, target_source_points)) 

            loss_mse= loss_mse_global+loss_mse_local

            cd_l1=cd_l1_global+cd_l1_local

            pca_reg_loss=0
            if model.coarse_to_fine:
                pca_reg_loss = mean_flat(criterion_mse(main_deformed_pts_global,interpo_points_basis_transformed))
            ...
            wandb.log({"cd_l1_global": cd_l1_global.item()}, step=global_step)
            wandb.log({"cd_l1_local": cd_l1_local.item()}, step=global_step)
            wandb.log({"loss_mse_mean": loss_mse_mean.item()}, step=global_step)
            wandb.log({"loss_mse_global": loss_mse_global.mean().item()}, step=global_step)
            wandb.log({"loss_mse_local": loss_mse_local.mean().item()}, step=global_step)
