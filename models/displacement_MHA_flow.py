...
class ImNet(nn.Module): # develop a transformation based IMNet?
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(nf * 16 + self.dimz, nf * 12)
        self.fc1to2 = nn.Linear(nf * 12 + self.dimz, nf * 8)
        ...
        self.fc = [self.fc0, self.fc1,self.fc1to2, self.fc2, self.fc3, self.fc4, self.fc5]
...
class GlobalToLocalProjection(nn.Module):
    def __init__(self, global_dim, local_dim, num_locals, num_heads, dropout=0.0):
        super().__init__()
        self.num_heads = num_heads
        self.num_locals = num_locals

        assert global_dim % num_heads == 0, "global_dim must be divisible by num_heads"
        head_dim = global_dim // num_heads
        self.scale = head_dim**-0.5
        self.to_queries = nn.Linear(global_dim, global_dim)

        self.pos_embedding = nn.Parameter(torch.randn(1, num_locals, global_dim))

        # self.to_keys_values = nn.Linear(dim_ref, dim_self * 2)
        self.to_keys_values = nn.Linear(global_dim, global_dim * 2)

        self.project = nn.Linear(global_dim, local_dim)
    ...
    def forward(
        ...
        # Remove redundant dimension if needed
        if global_vec.dim() == 3:
            global_vec = global_vec.squeeze(1)  # [batch_size, global_dim]


        # x = global_vec.unsqueeze(1)
        pos_enc = self.pos_embedding.expand(b, -1, -1)

        queries = self.to_queries(global_vec).reshape(
            b, 1, self.num_heads, -1
        )

        # 生成keys和values从position encoding
        # [batch_size, num_locals, 2, num_heads, head_dim]
        keys_values = self.to_keys_values(pos_enc).reshape(
            b, self.num_locals, 2, self.num_heads, -1
        )
        keys, values = keys_values[:, :, 0], keys_values[:, :, 1]
        
        # 计算注意力
        # [batch_size, 1, num_locals, num_heads]
        attention = torch.einsum('bqhd,bkhd->bqkh', queries, keys) * self.scale
        attention = attention.softmax(dim=2)
        attention = self.dropout(attention)
        
        # 聚合values得到local vectors
        # [batch_size, 1, num_heads, head_dim] -> [batch_size, num_locals, global_dim]
        # out = torch.einsum('bqkh,bkhd->bqhd', attention, values)
        out = torch.einsum('bqkh,bkhd->bkhd', attention, values)

        # print("show shape of out:",out.shape) # torch.Size([8, 1, 5, 4])
     
        out = out.reshape(b, self.num_locals, -1)
        
        # 投影到目标维度
        # [batch_size, num_locals, local_dim]
        local_vectors = self.project(out)
        
        return local_vectors, attention
...
class DeformationFlowNetwork(nn.Module):
    def __init__(
        ...
        self.use_hyperNet= hyperNet


        if self.use_hyperNet:
            self.hyper_global2local_net= GlobalToLocalProjection(
                global_dim=latent_size,
    ...
    def forward(self, latent_vector, points):
        ...
        def fwd_fn(points):
            ...
            # here i would like  to use a hypernetwork to handle the local information

            # print("show shape of local_vectors:",global_latent_vector.shape)# torch.Size([8, 1, 16])

            # if 
            # exit()
            local_vectors, attention = self.hyper_global2local_net(global_latent_vector)

            # print("show shape of local_vectors:",latent_vector.shape)# torch.Size([8, 1024, 16])
            # print("show shape of local_vectors:",local_vectors.shape)#  torch.Size([8, 4, 64])
            # local_vectors: torch.Size([8, 5, 64])
            # print("show shape of attention:",attention.shape)
            # attention: torch.Size([8, 1, 5, 4]
            # TODO: will add zero attention setup
            # exit()
            # print("mean_shape_point_labels:",self.mean_shape_point_labels.shape)

            # print("unique labels men_shape_point_labels:",self.mean_shape_point_labels.shape)
            unique_labels, label_counts = np.unique(self.mean_shape_point_labels, return_counts=True)
            # print("unique_labels:",unique_labels)# [0 1 2 3]
            # print("label_counts:", label_counts) # 282, 227，287，228
            # print("mean_shape_point_labels:", len(self.mean_shape_point_labels))
            # print("local_vectors:", local_vectors.shape[1])
            assert len(unique_labels)==local_vectors.shape[1], "number of clusters should be equal to the number of local latent vectors"

            points_latents = self.points_latents_assign(points, local_vectors, self.mean_shape_point_labels)
...
class Model(nn.Module):
    ...
    def forward(self, points, latent_sequence, pca_guidance_latents=None, shape_name= None):
        ...
        # add mean_shape_point_labels
        # self.net[0].add_lat_params(mean_shape_point_labels,'mean_shape_point_labels')

        # print("show shapeof mean_shape_point_labels:",self.net[0].mean_shape_point_labels.shape)
        # exit()
