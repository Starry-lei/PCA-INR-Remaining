...
from utils.model_utils import calc_cd_com
...
class PCTransformer(nn.Module):
    ...
    def forward(self, xyz):
        ...
        # x = self.encoder(xyz) # b n c

        print("show shape of x in forward:",x.shape)# 5, 256, 384
        # 
        ...
        # print("1show shape of coarse in forward:",coarse.shape)# 5, 256, 384

        

        coarse_inp = misc.fps(xyz, self.num_query//2) # B 128 3
        print("show shape of coarse_inp:",coarse_inp.shape)# 5, 512, 384
        coarse = torch.cat([coarse, coarse_inp], dim=1) # B 224+128 3?

        mem = self.mem_link(x)

        # print("2show shape of coarse in forward:",coarse.shape)# 5, 768, 3
        
        # print("show shape of mem in forward:",mem.shape)# 5, 256, 384
...
class Model(nn.Module):
    ...
    def build_loss_func(self):
        ...
        # self.loss_func = calc_cd_com()
        # cd_p, cd_t = calc_cd(pred, gt)

    def get_loss(self, ret, gt, epoch=1):
        pred_coarse, denoised_coarse, denoised_fine, pred_fine = ret

        # print("show pred_fine shape:",pred_fine.shape)# 20, 4096, 3
        # print("show gt shape:",gt.shape)# [20, 8192, 3]
        # exit()
        ...
        loss_denoised = calc_cd_com(denoised_fine, denoised_target)
        loss_denoised = loss_denoised * 0.5

        # recon loss
        loss_coarse = calc_cd_com(pred_coarse, gt)
        loss_fine = calc_cd_com(pred_fine, gt)
        loss_recon = loss_coarse + loss_fine

        return loss_denoised, loss_recon

    def forward(self, xyz, gt=None, is_training=True):

        print("show !!!!!!!!shape of xyz:",xyz.shape)
        ...
        # print("show !!!!!!!!shape of q:",q.shape)# [20, 576, 384]
        # print("show !!!!!!!!shape of coarse_point_cloud:",coarse_point_cloud.shape)# [20, 576, 3])
        # print("show !!!!!!!!shape of denoise_length:",denoise_length)# 64
        # exit()
