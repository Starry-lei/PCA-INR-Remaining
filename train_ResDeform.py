...
def train(args):
    ...
    for epoch in range(num_epochs):
        ...
        for data_tensors  in tqdm_train_loader:
            ...
            shape_idx, name, pca_theta, pca_recon, pca_input = data_tensors

            pca_theta= pca_theta.squeeze(1)
            res_flow_lat= flow_lat_params[shape_idx]
            print("show shape of pca_theta:",pca_theta.shape)#  torch.Size([4, 12])
            print("show shape of res_flow_lat:",res_flow_lat.shape)#torch.Size([4, 4, 12])
            ...
            # g_source_target_latents = torch.cat([mean_latents_thetlsa, source_thetas], dim=0)
            # g_target_source_latents = torch.cat([target_theta, mean_latents_theta], dim=0)
    
            deformed_pts = model(pca_recon, pca_theta, pca_res_latents=res_flow_lat, shape_name=name)  


            print("show shape of deformed_pts:",deformed_pts.shape)
